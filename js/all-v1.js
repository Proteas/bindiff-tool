...
function CommonFunctionFilter_V1(funcArray)
{
    retArray = []
    for (const func of funcArray) {
        ...
        // if (!func.getName(UDF_DataSelector_V1).includes("parse")) {
        //     continue
        // }
...
function OutputAll_V1()
{
    funcArray = BDT_Model.getFunction_WithActionMask(UDF_ActionType_Deleted | UDF_ActionType_Changed | UDF_ActionType_Identical)
    funcArray = CommonFunctionFilter_V1(funcArray)

    BDT_Outputer.setFunctionNameOutputFlag(FunctionNameOutputFlag_Tiny)
    BDT_Outputer.printFunctionsSummaryV1SortType(funcArray, FunctionSortType_WeaknessScore)
}
